          ...
          {testCases.map((tc, i) => {
            ...
            return (
              <div key={i} style={{ background:'#1c1f2e', border:`1px solid ${isOpen?cfg.color+'50':'#2d3142'}`, borderRadius:8, overflow:'hidden' }}>
                <div onClick={() => setExpanded(isOpen ? -1 : i)} style={{ padding:'12px 16px', cursor:'pointer', display:'flex', alignItems:'center', gap:10, background: isOpen?cfg.color+'10':'transparent' }}>
                ...
                {isOpen && (
                  <div>
                    ...
                    <div style={{ position:'relative' }}>
                      <button onClick={() => copy(i, tc.code)} style={{ position:'absolute', top:8, right:12, zIndex:1, background: copied===i?'#10b98120':'#1c1f2e', border:`1px solid ${copied===i?'#10b981':'#2d3142'}`, color: copied===i?'#10b981':'#6b7280', borderRadius:6, padding:'4px 10px', cursor:'pointer', fontSize:11 }}>
                        {copied===i ? '✅' : '📋'}
                      </button>
