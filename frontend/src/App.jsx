...
export default function App() {
  ...
  const loadSample = (lang) => { setLanguage(lang); setCode(lang === 'python' ? SAMPLE_PYTHON : SAMPLE_JAVA); setResult(null) }
      ...
      <div style={{ display:'flex', flex:1, overflow:'hidden' }}>
        ...
        <div style={{ width:'42%', display:'flex', flexDirection:'column', borderRight:'1px solid #2d3142', background:'#13161f' }}>
          <div style={{ padding:'14px 18px', borderBottom:'1px solid #2d3142', display:'flex', gap:8, alignItems:'center', flexWrap:'wrap' }}>
            ...
            <button onClick={handleAnalyze} disabled={loading} style={{ marginLeft:'auto', padding:'7px 20px', borderRadius:6, border:'none', background:loading?'#374151':'linear-gradient(135deg,#6366f1,#8b5cf6)', color:'#fff', cursor:loading?'not-allowed':'pointer', fontWeight:600, fontSize:14 }}>
              {loading ? '⏳ Analyzing...' : '⚡ Analyze'}
            </button>
          </div>
          <textarea value={code} onChange={e => setCode(e.target.value)} spellCheck={false}
            style={{ flex:1, background:'#0d1117', color:'#e1e4e8', border:'none', outline:'none', padding:'16px 20px', fontFamily:"'JetBrains Mono','Fira Code',monospace", fontSize:13, lineHeight:1.7, resize:'none' }}
          ...
          {!result && !loading && !error && (
            <div style={{ flex:1, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', gap:16 }}>
              ...
              <div style={{ fontSize:14, textAlign:'center', maxWidth:320, lineHeight:1.6, color:'#4b5563' }}>
                Paste code on the left, pick a language and mode, then click <strong style={{ color:'#818cf8' }}>⚡ Analyze</strong>
              </div>
              <div style={{ display:'flex', gap:12, marginTop:8 }}>
                {['🔍 Code Review','🧪 Test Generation','📚 RAG Context'].map(f => (
                  <div key={f} style={{ background:'#1c1f2e', border:'1px solid #2d3142', borderRadius:8, padding:'8px 14px', fontSize:12, color:'#9ca3af' }}>{f}</div>
                ))}
              </div>
            </div>
          )}
